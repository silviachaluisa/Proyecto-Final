const Mensaje = ({ children, tipo }) => {
    // children: Contenido del componente (Mensaje personalizado)
    // tipo: Tipo de mensaje (true: correcto, false: error)
    return (
        <div className={`p-6 border-l-4 ${tipo ? 'border-green-500' : 'border-red-500'} rounded-r-xl 
            ...
            <div className="ml-3">
                <div className={`text-sm ${tipo ? 'text-green-500' : 'text-red-500'}`}>
                    <p>{children}</p> {/* Contenido del componente (Mensaje personalizado)*/}
                </div>
            </div>
        </div>
    )
